...
% %
% % The postfix "_linear_extrapolate_interpolate" in the filename indicates the pretrained systems that are used for reconstruction is of linear interpolation and extrapolation
...
%           [state_after_step2, rc2] = do_longitudinal_prediction(A, A, LONGITUDINAL_SYSTEMS, n);
%           total_long_construct = total_long_construct + rc2;
%
%           % RESULTS = [RESULTS; {[state_after_step1, state_after_step2]}];
%           MEAN_LL = [MEAN_LL; {[state_after_step2]}];
%         end
%
        ...
        if (isnan(record(:,3)))
          ...
          switch (method)
            case  'FB'
              ...
              predicted_value_f = extrapolate(d,xf_1,xf_2,f1,f2);
              predicted_value_b = extrapolate(d,xb_1,xb_2,b1,b2);
              % predicted_value_intr = interpolate(d,xf_2,xb_2,f2,b2);
            ...
            case  'F'
              ...
              predicted_value = extrapolate(d,xf_1,xf_2,f1,f2);
            ...
            case 'B'
              ...
              predicted_value = extrapolate(d,xb_1,xb_2,b1,b2);
...
% XXXXXXXXXXXXXXXXXXXXXXXXXXX interpolate XXXXXXXXXXXXXXXXXXXXXXX
%
% XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
function out = interpolate(x,x1,x2,y1,y2)
  out = y1 + (x-x1)*(y2-y1)/(x2-x1);
end


% XXXXXXXXXXXXXXXXXXXXXXXXXXX extrapolate XXXXXXXXXXXXXXXXXXXXXXX
%
% XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
function out = extrapolate(x,x1,x2,y1,y2)
  out = y1 + ((x-x1)/(x2-x1))*(y2-y1);
end
