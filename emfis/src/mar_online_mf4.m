...
% sus_start
% keep the previous rules before updating the cluster
prev_net = net;
...
% %sus_start
    ...
    if (input_drift_detected)
        ...
        interpolation_output = sus_interpolation_extrapolation(data_input, net, prev_net, current_count);
    
        if (isnan(interpolation_output.y))         
            net.interpolation.output(current_count,1) = 0;      
        else
            net = interpolation_output.net;
            net.interpolation.output(current_count,1) = interpolation_output.y;
        end
...
%sus_start    
% set the interpolation output to zero if inter or extrapolation is not
% executed.
    if rule_updated ==false
        net.interpolation.output(current_count,1) = 0;
    end
%sus_end
