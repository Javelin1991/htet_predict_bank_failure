...
for i = 2:size(Data,1)   % iterating from 1st row to the last row
    ...
    for j = 1:no_Terms
        mfv(j) = exp( -(Data(i)-Terms(1,2*j-1))^2 / (Terms(1,2*j)^2) ); %find membership function value of each term
    end
            ...
            if Terms(1,2*j-1)-Data(i) < 0 
                ...
                if right_index == 0;
                    ...
                    if (Terms(1,2*j-1)-Data(i)) < (Terms(1,2*right_index-1)-Data(i))
                        right_index = j; %get the nearest right term
                    end
                end
            end
        end      
...
[centres_sorted,index] = sort(centres); %sort new index according to centre
