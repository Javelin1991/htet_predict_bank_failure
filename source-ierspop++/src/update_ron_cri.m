    ...
    for i = 1 : num_rules
        ...
        for j = 1 : num_attributes
            if net.rule(i).antecedent(j) == 0
                ...
                antecedent_values(i, j) = exp(-(data_input(j)-  net.input(j).mf(net.rule(i).antecedent(j)).params(1))^2/(net.input(j).mf(net.rule(i).antecedent(j)).params(2)^2));
...
%             centroid = (left_centroid + right_centroid) / 2;
            width = net.output(i).mf(b(j, 3)).params(2);
            centroid = net.output(i).mf(b(j, 3)).params(1);
