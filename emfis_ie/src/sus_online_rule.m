% XXXXXXXXXXXXXXXXXXXXXXXXXXX sus_online_rule XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
% 
% Author    :   Susanti
% Date      :   Aug 1 2014
% Function  :   Compute the points based on scale_rate
% Syntax    :   sus_online_rule(net, data_input, data_target,weight, current_count)
% 
...
% 4) If no same rule found, new rule generated with weight
% 5) All weights are normalized (see mar_normalize_weights)
% 
% XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX

function D = sus_online_rule(net, data_input, data_target,weight, current_count)
...
%    disp('Online Rule');
    ...
    for i = 1 : num_rules
        ...
        if sum(antecedent == net.rule(i).antecedent) == num_attributes && sum(consequent == net.rule(i).consequent) == num_outputs
            ...
            net.interpolation.rules(current_count,1).antecedent = antecedent;
            net.interpolation.rules(current_count,1).consequent = consequent;
            net.interpolation.rules(current_count,1).weight =  weight;
        ...
        elseif sum(antecedent == net.rule(i).antecedent) == num_attributes && sum(consequent == net.rule(i).consequent) ~= num_outputs
            ...
            if net.rule(i).weight < (weight)
                net.rule(i).weight = weight;            
    ...
    if rule_exists == 0 && inconsistent_found == 0
        ...
        net.rule(num_rules + 1).weight = weight;
